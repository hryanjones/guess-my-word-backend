...
curl -s -X POST "localhost:8080/leaderboard/2019-4-30/wordlist/normal?guesses=something,daughter&name=goobley&time=2000" \
    | grep -q "Date isn't the correct format"  || error_exit "date format 400"

curl -s -X POST "localhost:8080/leaderboard/2019-04-30/wordlist/notnormal?guesses=something,daughter&name=goobley&time=2000" \
    | grep -q "wordlist isn't one of known" || error_exit "wordlist 400"

## no name given
curl -s -X POST "localhost:8080/leaderboard/2019-04-30/wordlist/normal?guesses=something,daughter&time=2000" \
...
## over 24 hours time
curl -s -X POST "localhost:8080/leaderboard/2019-04-30/wordlist/normal?guesses=something,daughter&name=goobley&time=86400001" \
...
curl -s -X POST "localhost:8080/leaderboard/2019-04-30/wordlist/normal?guesses=daughter&name=goobley&time=1" \
    | grep -q "Sorry we don't accept" || error_exit "only one guess 400"
...
curl -s -X POST "localhost:8080/leaderboard/2019-04-30/wordlist/normal?guesses=something,daughter&name=mergen&time=100" > /dev/null
...
curl -s -X POST "localhost:8080/leaderboard/2019-05-01/wordlist/hard?guesses=something,aberration&name=mergen&time=100" > /dev/null
